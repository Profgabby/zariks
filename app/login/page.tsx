...
export default function LoginPage() {
  ...
  async function confirmFinanceAccess(supabase: ReturnType<typeof createClient>, userId: string) {
    const { data, error } = await supabase
      ...
      .select("user_id,active,workflow_role")
      .eq("user_id", userId)
      .maybeSingle();
    ...
    try {
      ...
      if (setupMode) {
        ...
        if (signUpData.session && signUpData.user) {
          const allowed = await confirmFinanceAccess(supabase, signUpData.user.id);
          if (!allowed) { await supabase.auth.signOut(); setMessage("This account is not active for ZARIKS Finance."); setLoading(false); return; }
          window.location.assign(destinationForUser(signUpData.user, nextPath));
          return;
        }
        setMessage("Account setup received. Sign in with the password you just created.");
        setSetupMode(false); setPassword(""); setLoading(false); return;
      }
      ...
      const allowed = await confirmFinanceAccess(supabase, data.user.id);
      if (!allowed) {
        await supabase.auth.signOut();
        setMessage("This account is not active for ZARIKS Finance. Contact the system administrator.");
        setLoading(false);
        return;
      }

      // Use a full navigation after auth so the SSR application reads the newly-written
      // Supabase cookies on a fresh request. This avoids client-router/cookie race loops.
      window.location.assign(destinationForUser(data.user, nextPath));
    } catch (error) {
