...
export default function LoginPage() {
  ...
  const [success,setSuccess]=useState(false); const [recoveryMode,setRecoveryMode]=useState(false); const [recoveryCode,setRecoveryCode]=useState(""); const [recoveryEmail,setRecoveryEmail]=useState("");
  ...
  async function handleForgotPassword(){
    ...
    if(error)setMessage(error.message);else{setRecoveryEmail(cleanEmail);setRecoveryMode(true);setSuccess(true);setMessage("A password-reset code has been sent. Enter the code from the newest email below.");}
    setResetLoading(false);
  }

  async function verifyRecoveryCode(){
    const token=recoveryCode.trim();
    if(!/^\d{6,8}$/.test(token)){setMessage("Enter the numeric recovery code from the newest email.");setSuccess(false);return;}
    setResetLoading(true);setMessage("");setSuccess(false);
    const supabase=createClient();
    const {data,error}=await supabase.auth.verifyOtp({email:recoveryEmail,token,type:"recovery"});
    if(error||!data.session){setMessage(error?.message??"The recovery code is invalid or expired. Request a new code.");setResetLoading(false);return;}
    window.location.replace(`/auth/update-password?recovery=1&next=${encodeURIComponent(currentNextPath())}`);
  }

  const initialMessage=currentMessage();
  return <main className="flex min-h-screen items-center justify-center bg-[#f5f7f5] p-6"><div className="w-full max-w-md overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-lg"><div className="bg-[#063d28] px-8 py-8 text-white"><div className="mb-5"><Image src="/zariks-logo.png" alt="ZARIKS Logo" width={72} height={72} priority className="h-[72px] w-[72px] rounded-2xl object-cover shadow-sm"/></div><h1 className="text-3xl font-bold tracking-wide">ZARIKS</h1><p className="mt-1 text-sm text-green-100">Transfer Control &amp; Accountability</p></div><form onSubmit={handleLogin} className="space-y-5 p-8"><div><h2 className="text-2xl font-bold">{setupMode?"Set up authorized account":"Sign in"}</h2><p className="mt-1 text-sm text-gray-500">{setupMode?"Only personnel already authorized in ZARIKS Finance can activate an account.":"Access the ZARIKS financial control system."}</p></div><label className="block"><span className="mb-2 block text-sm font-semibold">Email address</span><input type="email" required autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full rounded-lg border border-gray-300 px-4 py-3"/></label><label className="block"><span className="mb-2 block text-sm font-semibold">Password</span><input type="password" required autoComplete={setupMode?"new-password":"current-password"} value={password} onChange={e=>setPassword(e.target.value)} className="w-full rounded-lg border border-gray-300 px-4 py-3"/></label>{!setupMode&&!recoveryMode&&<button type="button" onClick={handleForgotPassword} disabled={resetLoading} className="text-sm font-semibold text-[#006b3c] hover:underline">{resetLoading?"Sending code...":"Forgot password?"}</button>}{!setupMode&&recoveryMode&&<div className="space-y-3 rounded-lg border border-green-100 bg-green-50 p-4"><p className="text-sm font-semibold text-[#006b3c]">Enter password-reset code</p><p className="text-xs text-gray-600">Use the numeric code sent to {recoveryEmail}.</p><input inputMode="numeric" autoComplete="one-time-code" value={recoveryCode} onChange={e=>setRecoveryCode(e.target.value.replace(/\D/g,""))} placeholder="Recovery code" className="w-full rounded-lg border border-gray-300 bg-white px-4 py-3"/><button type="button" onClick={verifyRecoveryCode} disabled={resetLoading||!recoveryCode} className="w-full rounded-lg bg-[#006b3c] px-5 py-3 font-semibold text-white disabled:opacity-60">{resetLoading?"Verifying...":"Verify code & continue"}</button><button type="button" onClick={handleForgotPassword} disabled={resetLoading} className="w-full text-sm font-semibold text-[#006b3c] hover:underline">Send a new code</button></div>}{(message||initialMessage)&&<div className={`rounded-lg border p-3 text-sm ${success?"border-green-100 bg-green-50 text-green-800":"border-red-100 bg-red-50 text-red-700"}`}>{message||initialMessage}</div>}<button type="submit" disabled={loading} className="w-full rounded-lg bg-[#006b3c] px-5 py-3 font-semibold text-white disabled:opacity-60">{loading?"Signing in...":setupMode?"Activate account & continue":"Sign in"}</button><button type="button" onClick={()=>{setSetupMode(v=>!v);setMessage("");setSuccess(false);setPassword("");}} className="w-full text-center text-sm font-semibold text-[#006b3c] hover:underline">{setupMode?"Already have an account? Sign in":"Authorized personnel: activate account"}</button></form></div></main>;
}