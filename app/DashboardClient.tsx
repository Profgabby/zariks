...
export default function DashboardClient({user}:{user:{email:string;fullName:string;role:string}}){
 ...
 return <main className="min-h-screen bg-[#f5f7f5] text-[#152019]">
  ...
  <section className="mx-auto max-w-7xl px-6 py-8 md:px-8">
   ...
   <div className="mb-8 grid gap-5 md:grid-cols-4"><QuickAction title="Requests" description="Create and track financial authorization before money moves." href="/procurement" action="Open requests →"/><QuickAction title="Payroll & Scheduled Payments" description="See approved recurring salaries, commissions and allowances due this week or month and generate the next payment." href="/payroll" action="Open payroll →"/><QuickAction title="Payments & Transfers" description="Process actual disbursements created after final approval or from an approved schedule." href="/transfers" action="View payments →"/><QuickAction title="Recipient Confirmation" description="Review payments awaiting receipt or certification." href="/mediatrix" action="Open confirmations →"/></div>
   <div className="mb-8 rounded-xl border bg-white p-6 shadow-sm"><div className="mb-5"><h3 className="text-lg font-bold">ZARIKS Financial Control Flow</h3><p className="text-sm text-gray-500">One-time requests proceed to payment after approval. Approved recurring requests become schedules and generate future payment records when due.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8"><WorkflowStep number="1" label="Request"/><WorkflowStep number="2" label="Verify"/><WorkflowStep number="3" label="Finance Review"/><WorkflowStep number="4" label="Final Approval"/><WorkflowStep number="5" label="Schedule / Pay"/><WorkflowStep number="6" label="Receipt / Certify"/><WorkflowStep number="7" label="Retire"/><WorkflowStep number="8" label="Close"/></div></div>
   <div className="overflow-hidden rounded-xl border bg-white shadow-sm"><div className="flex items-center justify-between border-b px-6 py-5"><div><h3 className="font-bold">Recent Payments &amp; Fund Transfers</h3><p className="text-sm text-gray-500">Actual money movements will appear here after approved requests or scheduled obligations are paid.</p></div><Link href="/transfers" className="text-sm font-semibold text-[#006b3c]">View all →</Link></div><div className="px-6 py-10 text-center text-sm text-gray-500">No payments or fund transfers recorded yet.</div></div>
  </section>
 </main>
}
function Metric({label,value}:{label:string;value:string}){return <div className="rounded-xl border bg-white p-5 shadow-sm"><p className="text-sm text-gray-500">{label}</p><p className="mt-2 text-2xl font-bold">{value}</p></div>}
function WorkflowStep({number,label}:{number:string;label:string}){return <div className="rounded-lg border border-green-100 bg-green-50 p-4"><div className="mb-3 flex h-8 w-8 items-center justify-center rounded-full bg-[#006b3c] text-xs font-bold text-white">{number}</div><p className="text-sm font-semibold">{label}</p></div>}
function QuickAction({title,description,href,action}:{title:string;description:string;href:string;action:string}){return <div className="rounded-xl border bg-white p-6 shadow-sm"><h3 className="font-bold">{title}</h3><p className="mt-2 text-sm text-gray-500">{description}</p><Link href={href} className="mt-4 inline-block text-sm font-semibold text-[#006b3c]">{action}</Link></div>}
