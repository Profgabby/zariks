...
export async function createProcurementRequest(formData: FormData) {
  ...
  const items = itemNames.map((item_name, index) => ({
    item_name: item_name.trim(),
    description: (descriptions[index] ?? "").trim(),
    quantity: quantities[index],
    unit: (units[index] ?? "").trim(),
    unit_cost: unitCosts[index],
  })).filter((item) => item.item_name && Number.isFinite(item.quantity) && item.quantity > 0 && Number.isFinite(item.unit_cost) && item.unit_cost >= 0);
  ...
  const { data, error } = await supabase.rpc("fn_create_procurement_request", {
    p_request_type: text(formData, "request_type") || "procurement",
    p_request_date: text(formData, "request_date") || new Date().toISOString().slice(0, 10),
    p_from_whom: text(formData, "from_whom"),
    p_to_whom: text(formData, "to_whom"),
    p_department: text(formData, "department"),
    p_purpose: text(formData, "purpose"),
    p_justification: text(formData, "justification"),
    p_required_by: text(formData, "required_by") || null,
    p_items: items,
    p_submit: true,
  });
