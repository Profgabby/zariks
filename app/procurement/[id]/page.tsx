...
export default async function ProcurementDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: request } = await supabase.from("procurement_requests").select("*").eq("id", id).single();
  if (!request) notFound();
  ...
  return <main className="min-h-screen bg-[#f5f7f5] px-5 py-8 text-[#152019]"><div className="mx-auto max-w-5xl">
    ...
    <article className="mt-6 overflow-hidden rounded-2xl border bg-white shadow-sm">
      <header className="bg-[#063d28] p-7 text-white"><p className="text-sm font-bold uppercase tracking-wider text-[#FDB515]">ZARIKS™ Cashflow Control</p><div className="mt-2 flex flex-col justify-between gap-3 sm:flex-row"><div><h1 className="text-3xl font-bold">Money / Procurement Request</h1><p className="mt-1 text-green-100">{request.request_no}</p></div><div className="text-left sm:text-right"><p className="text-sm text-green-100">TOTAL</p><p className="text-3xl font-bold text-[#FDB515]">{ngn.format(Number(request.total_amount))}</p></div></div></header>
      <div className="space-y-8 p-7">
        <section className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4"><Info label="Date" value={request.request_date} /><Info label="Type" value={String(request.request_type).replaceAll("_"," ")} /><Info label="From Whom" value={request.from_whom || "—"} /><Info label="To Whom" value={request.to_whom || "—"} /><Info label="Department" value={request.department || "—"} /><Info label="Required By" value={request.required_by || "—"} /><Info label="Status" value={String(request.status).replaceAll("_"," ")} /><Info label="Currency" value="Nigerian Naira (NGN)" /></section>
...
function Info({label,value}:{label:string,value:string}) { return <div><p className="text-xs font-bold uppercase tracking-wider text-gray-500">{label}</p><p className="mt-1 font-semibold capitalize">{value}</p></div> }
function Sign({label,value,date}:{label:string,value:string,date:string|null}) { return <div className="rounded-xl border p-4"><p className="text-xs font-bold tracking-wider text-[#006b3c]">{label}</p><p className="mt-3 font-bold">{value}</p><p className="mt-1 text-xs text-gray-500">{date ? new Date(date).toLocaleString("en-NG") : "Awaiting action"}</p></div> }
